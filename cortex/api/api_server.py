...
def run_api_server(host='localhost', port='5000', database_url='mongodb://localhost:27017'):
    ...
    def get_user_snapshots(id):
        result = users.find_one({"user_id": id}, {
                                "snapshots.snapshot_id": 1, "snapshots.datetime": 1, "_id": 0})
        if not result:
            return make_response('User not found\n', 404)
    ...
    def get_snapshot(user_id, snap_id):
        result = users.find_one({"user_id": user_id, "snapshots.snapshot_id": snap_id}, {
                                "_id": 0, "snapshots": {"$elemMatch": {"snapshot_id": snap_id}}})
        if not result:
            return make_response('Snapshot not found\n', 404)

        snapshot = result['snapshots'][0]
        observations = [key for key in snapshot if key not in [
            'snapshot_id', 'datetime']]
        json_result = jsonify(
            {'snapshot_id': snapshot['snapshot_id'], 'datetime': snapshot['datetime'], 'results': observations})
    ...
    def get_result(user_id, snap_id, result_name):
        query_result = users.find_one({"user_id": user_id, "snapshots.snapshot_id": snap_id, f'snapshots.{result_name}': {
                                      "$exists": True}}, {"_id": 0, "snapshots": {"$elemMatch": {"snapshot_id": snap_id}}})
        if not query_result:
    ...
    def get_result_data(user_id, snap_id, result_name):
        query_result = users.find_one({"user_id": user_id, "snapshots.snapshot_id": snap_id, f'snapshots.{result_name}.data': {
                                      "$exists": True}}, {"_id": 0, "snapshots": {"$elemMatch": {"snapshot_id": snap_id}}})
        if not query_result:
